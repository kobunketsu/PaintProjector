...
+ (id)materialsFromMtlFile:(NSString *)path
{
	...
	for (int i = 0; i < [mtlLines count]; i++)
	{
		...
		if ([line hasPrefix:@"newmtl"]) // Start of new material
		{
			...
			{
				NSString *line = [mtlLines objectAtIndex:j];
                NSString *parseLine = line;
                if([line length]>0)
                    parseLine = [parseLine substringFromIndex:1];
				// ignore Ni, d, and illum, and texture - at least for now
				if ([line hasPrefix:@"newmtl "])
					material.name = [line substringFromIndex:7];
				...
				else if ([parseLine hasPrefix:@"map_Kd "])
				{
					...
					for (int k=4; k <= 1024; k*=2)
					{
						...
						NSString *newBase = [NSString stringWithFormat:@"%@-%d", baseName, k];
