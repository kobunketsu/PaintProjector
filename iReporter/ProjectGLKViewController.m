...
@interface ProjectGLKViewController ()

@end



@implementation ProjectGLKViewController
...
- (void)viewDidLoad
{
    ...
    [delegate createPaintProjectEAGleContext:self.context];
    if (!self.context) {
...
- (void)initPaint:(PaintingView*)paintView viewAngle:(float)angle{
    ...
    [self setManHeight: 1.75];
    _eye = GLKVector3Make(0, [self manHeight], 0);
    [self setEyeT: 0];
...
- (void)setupRealQuadParams{
    ...
    //实际地面绘制长宽
    _projHeight = fabsf([self farDistance] - nearDistance);
    float tanFovXHalf =tanf(fovYHalf) * _aspect;
    _projWidth = _projHeight * tanFovXHalf * 2;
...
- (void)setupLineParams{
    ...
    else if(_state == Projecting || _state == UnProjecting)
    {
        ...
        eye = GLKVector3Lerp(_eye, _eyeTop, [self eyeT]);
        center = GLKVector3Lerp(_projFocus, _projCenter, [self eyeT]);
    }
